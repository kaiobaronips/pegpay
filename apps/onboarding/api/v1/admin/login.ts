...
import { createTotpSecret, isMissingMfaSchema, otpauthUri, startMfaChallenge } from '../../../server/mfa.js'
...
export default async function handler(request: ApiRequest, response: ServerResponse): Promise<void> {
  ...
  try {
    ...
    let mfa: { enabled_at: string | null }[]
    try {
      mfa = await sql`SELECT enabled_at FROM admin_mfa_credentials WHERE email = ${email} LIMIT 1` as { enabled_at: string | null }[]
    } catch (error) {
      if (!isMissingMfaSchema(error)) throw error
