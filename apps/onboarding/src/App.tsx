...
function CustomerPortalV2() {
  ...
  useEffect(() => () => window.clearTimeout(cepTimer.current), [])
  ...
  function updateZipCode(value: string) {
    ...
    // Sem espera, cada tecla que completa 8 dígitos dispara uma consulta — apagar e redigitar
    // um número gerava várias. Quem digita rápido agora gasta uma chamada, não cinco.
    window.clearTimeout(cepTimer.current)
    if (cep.length === 8) cepTimer.current = window.setTimeout(() => void lookupCep(cep), 400)
  }
  ...
  if (successProtocol) {
    const statusCopy: Record<Exclude<ProposalStatus, 'DRAFT'>, [string, string]> = { RECEIVED: ['CADASTRO FINALIZADO', 'Cadastro enviado com segurança.'], UNDER_REVIEW: ['EM ANÁLISE', 'Sua proposta está sendo analisada.'], PENDING: ['PENDÊNCIA', 'A proposta precisa de informações adicionais. Fale com a PegPay.'], APPROVED: ['PROPOSTA APROVADA', 'Sua proposta foi aprovada. Aguarde as orientações da PegPay.'], REJECTED: ['PROPOSTA NÃO APROVADA', 'Desta vez, não conseguimos aprovar sua proposta.'] }
    const copy = session?.status && session.status !== 'DRAFT' ? statusCopy[session.status] : statusCopy.RECEIVED
    const received = copy[0] === 'CADASTRO FINALIZADO'
    const rejected = copy[0] === 'PROPOSTA NÃO APROVADA'
    return <><Header /><main className="state-page success-page"><div className="eyebrow">{copy[0]}</div><h1>{copy[1]}</h1><p>Protocolo: <strong className="tnum">{successProtocol}</strong></p>{received ? <><p>Obrigado por chegar até aqui conosco.<br />Você receberá atualizações da sua proposta diretamente no seu WhatsApp.<br />Para mais dúvidas, entre em contato pelo e-mail <a href="mailto:proposta@pegpay.com.br">proposta@pegpay.com.br</a>.</p><aside className="notice"><b>O envio não representa aprovação.</b></aside></> : rejected ? <><p>Sabemos que essa não era a resposta que você esperava.</p><p>Cada solicitação passa por uma análise individual e, neste momento, não foi possível seguir com a contratação.</p><p>Se quiser esclarecer dúvidas sobre o resultado, escreva para <a href="mailto:proposta@pegpay.com.br">proposta@pegpay.com.br</a> e informe o protocolo acima.</p></> : <p>Para acompanhamento, fale com contato@pegpay.com.br. O envio não representa aprovação.</p>}</main></>
  }
