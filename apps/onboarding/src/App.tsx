...
function CustomerPortal() {
  ...
  const [form, setForm] = useState({ fullName: '', cpf: '', birthDate: '', email: '', bankName: '', bankBranch: '', bankAccount: '', bankAccountType: 'corrente', consent: false })
  ...
  const ready = hasSelfie && hasDocumentFront && hasDocumentBack && form.fullName && form.cpf && form.birthDate && form.email && form.bankName && form.bankBranch && form.bankAccount && form.consent
...
function AdminPortal() {
  ...
  useEffect(() => { void load() }, [])
  async function login(event: FormEvent) { event.preventDefault(); setMessage(''); try { await api<{ email: string }>('/api/v1/admin/login', { method: 'POST', body: JSON.stringify({ email, password }) }); await load() } catch (error) { setMessage(error instanceof Error ? error.message : 'Falha no login.') } }
