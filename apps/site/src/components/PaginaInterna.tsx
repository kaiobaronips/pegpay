...
interface PaginaInternaProps {
  ...
  atualizadoEm?: string;
  /** Aplica a hero laranja da identidade PegPay sem afetar outras páginas internas. */
  heroLaranja?: boolean;
  /**
   * Sobrescreve o texto e o destino do CTA final ("Ficou com alguma
   * dúvida?"). Sem isso, cai no padrão — WhatsApp oficial. Usado pela
   * página de Privacidade, cujo assunto (direitos de dados) tem canal
   * próprio, diferente do atendimento geral.
   */
  ctaTitulo?: string;
  ctaTexto?: string;
  ctaHref?: string;
...
export default function PaginaInterna({
  ...
  useSeo(`${kicker ?? titulo} — PegPay`, descricaoSeo ?? resumo, pathname);
  ...
  return (
    <div className="min-h-screen bg-paper font-archivo text-ink">
      ...
      <main className="pt-[68px]">
        <section className={cn(heroLaranja ? "bg-peg" : "rule-b bg-paper")}>
          <div className="mx-auto max-w-[1200px] px-5 py-16 md:px-8 md:py-20">
            {kicker && (
              <p className={cn("label", heroLaranja ? "text-ink" : "text-peg")}>{kicker}</p>
            )}
            <h1
              className={cn(
                "mt-4 max-w-[18ch] whitespace-pre-line font-archivo text-[42px] font-extrabold leading-[1.02] tracking-[-0.035em] md:text-[64px]",
                heroLaranja && "text-paper",
              )}
            >
              {titulo}
            </h1>
            <p
              className={cn(
                "mt-6 max-w-[62ch] text-[17px] font-medium leading-relaxed",
                heroLaranja ? "text-ink/80" : "text-ink/70",
              )}
            >
              {resumo}
            </p>
            {atualizadoEm && (
              <p className={cn("label mt-8", heroLaranja ? "text-ink/65" : "text-ink/45")}>
          ...
          <div className="mx-auto flex max-w-[1200px] flex-col gap-6 px-5 py-16 md:flex-row md:items-center md:justify-between md:px-8">
            <div>
              <h2 className="max-w-[20ch] font-archivo text-[30px] font-extrabold leading-[1.05] tracking-[-0.025em] md:text-[38px]">
                {ctaTitulo ?? "Ficou com alguma dúvida?"}
              </h2>
            ...
            <a
              ...
              {...(!ctaHref || ctaHref === WHATSAPP_URL ? { target: "_blank", rel: "noreferrer" } : {})}
...
export function Bloco({
  numero,
  titulo,
  semLinhaSuperior = false,
  children,
}: {
  numero: string;
  titulo: string;
  semLinhaSuperior?: boolean;
  children: ReactNode;
}) {
  return (
    <article
      className={`${semLinhaSuperior ? "" : "rule-t "}grid gap-4 py-9 first:pt-0 md:grid-cols-[220px_1fr] md:gap-10`}
    >
